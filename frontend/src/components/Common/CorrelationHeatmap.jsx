...
function getCorrelationColor(value) {
  ...
  if (v >= 0.2) return 'bg-green-300';
  if (v >= -0.2) return 'bg-gray-200';
      ...
      <table className="w-full text-sm">
        <thead>
          <tr>
            ...
            {colLabels.map(c => (
              <th key={c} className="py-2 px-2 text-center text-xs font-semibold text-gray-600">{c}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {rowLabels.map(r => (
            <tr key={r}>
              <td className="py-1 px-2 text-xs font-semibold text-gray-600 text-right whitespace-nowrap">{r}</td>
              {colLabels.map(c => {
